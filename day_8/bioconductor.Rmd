...
```{r}
kevin@age # don't do that --> Note: because you could end up in places where the dev doesn't want you to go, could break things. Using @ like this is a Seurat thing. If you are supposed to be able to do it then the dev will have provided a function to do it. 
```
...
> Answer: 41 packages out of date.. wow. But can't update as cannot install packages on CCB cluster, not enough permissions!
> 
...
help(DNA_ALPHABET)
IUPAC_CODE_MAP
# IUPAC_Code, combined letters mean that nucleic acid in that position could be any one of those.

help("AA_ALPHABET") # Amino acid alphabet available too
```
...
```{r}
IlluminaQuality(0:40) # lower case letters indicative of quality, higher the better
PhredQuality(0:40)
SolexaQuality(0:40)
...
```{r, eval=FALSE}
browseVignettes("Biostrings") # These links don't work when using R on the server but will work when using the RStudio on own computer
```
...
```{r}
file.edit("/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/TruSeq3-PE-2.fa") # Can check to see how file looks
...
- Use the function `BSgenome::available.genomes()`.
  What does it do?
Note: Shows available genome data. Reference genomes for testing, comparing and practicing with. Masked means..?
```{r}
...
genome
```
Note: _alt indicates that it contains alternative sequences, so all sequencing reads can have a similar chance to bind. Displaying genome usually only displays a small number of genes, depending on size of panel.
...
note: countPattern needs fixed = FALSE if using something like N, as N means any nucleotide, but N will not ACTUALLY be present in the sequencing data
```{r}
Ebox <- "CANNTG"
...
> Answer: Like a factor, info stored in a similar way
> 
...
## Metadata columns
Note: Metadata cols can be named anything, so function cannot be written around it. So need to access in a different way
...
```
Note: This command is a bit confusing, you need mcols to grab the metadata, not this one!
```{r}
...
## DataFrame
Note: Bioconductor has it's own data frame, need to specify as DataFrame. ggplot does not work with this one! But does behave in a similar way to other data frames at least.
...
demo_DataFrame
```
Note: Giving metadata on the columns, a DataFrame can have metadata on it's columns! Special trick of Bioconductor data frames, if converted by as.dataframe etc it would lose this metadata
```{r}
...
Note: Tracks are green sections on the UCSC genome browser. Each track imports a different data type in a different way. rtracklayer can import these different files into bioconductor
...
```{r}
"/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/Homo_sapiens.GRCh38.104.gtf.gz"
...
```{r}
mcols(gtf_data) # All cols including metadata
colnames(mcols(gtf_data)) # Returns only metadata columns, less info and less overwhelming
...
gtf_ACTB #Object ranges has greatly reduced and gene_name is listed as ACTB
```

- How many transcripts do the annotations describe for that gene?

```{r}
transcript_ACTB <- unique(gtf_ACTB$transcript_id)
transcript_ACTB <- na.omit(transcript_ACTB) # Removing first NA value as it will still count the NA as a value
length(transcript_ACTB)
...
# This divides the data in the vector x into the groups defined y f. So pulling out every exon 
gtf_ACTB_transcripts # Can tell it's worked because we now have a GRangesList (before was just GRanges)
```
...
```{r}
human_symbols <- keys(org.Hs.eg.db, keytype = "SYMBOL")[1:6]

human_symbols
```

- Use the `AnnotationDbi::select()` method to fetch the Entrez and Ensembl identifiers for those gene symbols.
Note: need to specify package with :: as the multiple select functions do not play well together!
Also --> Here we are selecting the ENTREZID and ENSEMBL data for the 6 data points we have already pulled from SYMBOL in the human-symbols object.
```{r}
AnnotationDbi::select(org.Hs.eg.db, human_symbols, c("ENTREZID", "ENSEMBL"), keytype = "SYMBOL")
```

  + What happens if you do not specify `AnnotationDbi`?

> Answer: It will call the select function from a different package, might not happen if packages are loaded in a set order, but best to avoid!
> 

- Use the `mapIds()` method to get the Ensembl identifier for those gene symbols.
Note: mapIDs() --> This value means that when there are multiple matches only the 1st thing that comes back will be returned. This is the default behavior
Returns as a vector as if there is duplicated data then it will incorrectly shift results down. No matter what mapIDs will return a vector of the correct length
Can also call specific genes in data set (replace where human_symbols is)
```{r}
mapIds(org.Hs.eg.db, human_symbols, "ENSEMBL", "SYMBOL")
mapIds(org.Hs.eg.db, human_symbols, "ENSEMBL", "SYMBOL", multiVals = "list") # To create a list output
```
...
Note: Lists ensembl packages that it is available today from BioMart website
...
```{r}
mart <- useMart(biomart = "ENSEMBL_MART_ENSEMBL", dataset = "hsapiens_gene_ensembl", host ="https://may2021.archive.ensembl.org"
)
mart
```
...
```{r}
mart_attri <- listAttributes(mart)
View(mart_attri)
```
...
```{r}
getBM(
 attributes =  c("hgnc_symbol", "chromosome_name","start_position", "end_position", "strand"), filters = "hgnc_symbol", values = human_symbols, mart = mart
)

listFilters(mart) # Used to try and find appropriate filter (also easier to find in the pop up mart_attributes list using View()!)
```
...
```{r}
assays(demo_se)
assayNames(demo_se) # Can be useful for looping over assays
assay(demo_se, "counts") # Extract one assay's data
colData(demo_se) # Extract col data
rowData(demo_se)
mcols(demo_se) # Same as rowData(), avoid because can go wrong
metadata(demo_se) # Empty list because no metadata has been defined
```

What do those functions do?

> Answer: All sorts of magical things
> 
...
row.names(assay_counts) <-  assay_counts$X
assay_counts$X <- NULL # baseR as way to delete a column
...
```{r}
sample_metadata <- read.csv("/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/summarized_experiment/sample_metadata.csv",row.names = 1) # row.names = 1 takes the first column and use that as the row names (can be changed to other col)
