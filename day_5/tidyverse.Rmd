...
> Answer: Item has 150 rows, 5 columns, 2 different data types, col names... 
...
> Answer:A tibble. 150 x5. Data are dbl (double precision number) and one col of characters.
...
> Answer: We have lost combined data for each individual flower, e.g. flower 1 had width x and height y
...
iris_long2 <- iris_raw %>% 
  ...
  select(!c(ID, species)) %>% 
  pivot_longer(!species_ID, names_to = "variable", values_to = "value")
...
iris_wide
# By not naming the species_ID, tidy will automatically try to categorise by the unamed value (?)
...
exprs_assay <- read_xlsx("/project/obds/shared/resources/2_r/r_tidyverse/EH2011.xlsx", sheet = "exprs")
...
exprs_assay_long <- exprs_assay %>% 
    pivot_longer(!gene, names_to = "sample", values_to = "gene_expression")
...
exprs_sampleinfo %>% 
  group_by(sex, gene) %>% 
  summarise(gene_expression = mean(gene_expression)) %>% 
  pivot_wider(names_from = sex, values_from = gene_expression) %>% 
