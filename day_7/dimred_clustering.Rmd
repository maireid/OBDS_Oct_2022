...
Note- Matrix so have to remove characters in object (a.k.a. species)
```{r}
iris_features <- iris_data %>% select(!Species)
...
# To see a more detailed read out of the PCA
str(pca_iris)

# Loading of each feature = calling specific string of data from the PCA
pca_iris$rotation
...
```{r}
pca_iris_dataframe <-  cbind(pca_iris_dataframe, "Species" = iris_data$Species)
head(pca_iris_dataframe)
```
...
```{r}
ggplot(pca_iris_dataframe, aes(x = PC1, y = PC2, shape = Species, color = Petal.Length)) +
  geom_point(size = 3)

```

> Answer:
> Petal length corresponds with PC1
> 
...
Note: No idea what happened here, based on a calculation from somewhere? Have converted values into percentage to help visualise... something? PC3 and PC4 have low values so can be discarded (dimensionality reduction)
...
variance_dataframe <- data.frame(
  name = names(explained_variance_ratio),
...
```{r}
pca_variance <- ggplot(variance_dataframe, aes(x = name, y = variance, fill = name))+
  geom_col()
  pca_variance
```
...
colnames(tsne_iris_dataframe) <- paste0("TSNE", 
        seq_len(ncol(tsne_iris_dataframe)))
...
```{r}
tsne_iris_species <- ggplot(tsne_iris_dataframe, aes(x = TSNE1, y = TSNE2, color = Species)) +
  geom_point()
...
cowplot::plot_grid(pca_variance, umap_iris_species, tsne_iris_species, ncol = 2, nrow = 2)
...
> Answer:
> Definitely 2 clusters (main branches), total 3 or potentially 4.
> 
...
Note: EITHER k or h needs to be separated, don't need both
...
Note: cluster 0 is the way for dbscan to indicate excluded unclustered values, not it's own clusters!
...
cowplot::plot_grid(dbscan_plot, hdbscan_plot, ncol = 2)
